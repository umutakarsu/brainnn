"""Tactile modality encoder/decoder — pressure/texture pattern processing."""
...
class TactileEncoder(BaseModalityEncoder):
    """2D convolutional encoder for tactile pressure maps.

    Input:  (batch, 1, H, W)  — e.g. (B, 1, 32, 32) pressure grid
...
class TactileDecoder(BaseModalityDecoder):
    """Decoder that reconstructs tactile pressure maps from latent representation.

    Input:  (batch, latent_dim)
    Output: (batch, 1, H, W)
    """

    def __init__(self, config: ModalityConfig, output_size: int = 32) -> None:
        super().__init__(config)
